...
| Services                    | CURRENT: structured offerings with full editing, optional duration, explicit pricing, active/inactive state and guarded admin-only catalog deletion.                                                                                                                                        |
| Conversation                | CURRENT: channel-agnostic interaction history, with messages and activity events.                                                                                                                                                                                                           |
| Service Request             | CURRENT: minimal primary work object with structured wants/known/missing summary, lifecycle, attention and next action, plus optional customer/service/conversation/booking links. Removed-service context is preserved without a broken live catalog ID. Full intake-to-quote workflow remains PLANNED. |
...
Each tenant receives separate Business Profile, Business Hours and AI Policy records during repeat-safe tenant bootstrap. Members can read settings; only verified `org:admin` users can change them. Business identity includes company name, timezone, default language and optional contact/address/description fields. Weekly hours support closed days and multiple non-overlapping intervals. Business Hours and resource schedules remain independently configured; both constrain ordinary booking availability, while conflicts and capacity remain separate checks. Holiday and buffer rules remain planned.
...
The authenticated operator calendar uses FullCalendar React 7.1 Standard under its MIT license. Desktop has day/week views and drag-to-reschedule with fixed resource and duration; mobile has a daily agenda without horizontal overflow at the verified 390 px viewport. Staff create bookings from existing or new customers, select a service and one resource, optionally link an existing Service Request, then view customer/contact details, reschedule, complete or cancel the booking. Every action also works without drag-and-drop. Cancelled bookings are hidden by default and can be included explicitly. The calendar component displays data and opens our shadcn dialogs; Convex owns authorization and booking rules.

Resources are tenant-scoped people, rooms or equipment with capacity one, active/inactive status, an explicit all-services or selected-services restriction (where an empty selected list means none), their own weekly schedules and explicit blocked intervals. Resource configuration is admin-only; verified members can book. Business opening hours and resource schedules remain separate data but both constrain ordinary availability. Staff may explicitly confirm an audited booking outside either weekly schedule; the first attempt is a normal typed `needs_confirmation` result with no writes, and each confirmation is bound to the exact time/resource/version attempt. AI and ordinary Tool Layer paths cannot request that capability. Blocks, conflicts, tenant ownership, active status and service eligibility are never overridden. Administrators may copy current business hours into a resource schedule once; no ongoing synchronization is implied. New bookings require a configured resource schedule. When AI omits a resource, the server chooses the first eligible available resource deterministically by Swedish name order and ID.

Services may be edited across name, description, duration and pricing. Missing duration marks a service as not ready for calendar booking while keeping informational services valid. Admins may permanently delete an active or inactive catalog service when no confirmed/oavslutad booking references it. Completed and cancelled bookings remain unchanged through stored snapshots. Service Requests remain open with preserved service context but lose the live catalog ID; a later booking requires a current bookable service. Service-resource links are removed while resources remain, and removing the last selected link never broadens access to all services. Preview scope is rechecked and all history/link handling plus deletion commits in one transaction; oversized relation sets fail without partial changes. Optional snapshot and restriction-mode fields use legacy inference, so no bulk migration is required.
