...
| Services                    | CURRENT: structured offerings with full editing, optional duration, explicit pricing, active/inactive state and guarded permanent deletion of unused mistakes.                                                                                                                              |
...
Services may be edited across name, description, duration and pricing. Missing duration marks a service as not ready for calendar booking while keeping informational services valid. Admins may permanently delete an active or inactive service only when no booking (including cancelled/history), Service Request or resource restriction references it. The check and delete are one transaction; related data is never cascaded or detached automatically.
