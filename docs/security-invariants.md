...
- Permanent service deletion MUST require verified `org:admin`, derive tenant server-side and check every booking (including cancelled/history), Service Request and resource link in the same mutation. Any reference MUST reject deletion without cascade or automatic unlinking. Existing edit/status permissions remain unchanged.
