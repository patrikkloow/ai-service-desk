...
- Calendar reschedules MUST carry a server-issued booking version and reject stale writes in the mutation. Expected weekly-schedule confirmation MUST be a typed no-write result, and each confirmation MUST be bound to one immutable booking/time/resource/version attempt. Rapid reentry and late callbacks MUST be isolated per booking. UI drag failures MUST revert or reconcile with authoritative server state and MUST NOT retry blindly. Event resizing is disabled; duration and resource stay unchanged during drag.
...
- Permanent service deletion MUST require verified `org:admin`, derive tenant server-side and reject every confirmed/oavslutad booking in the deleting transaction. Completed/cancelled bookings MUST remain intact through stored snapshots. Service Requests MUST remain intact with service context preserved and their deleted live catalog ID cleared. Only service-resource join rows may be removed; resources MUST remain and explicit selected-with-none semantics MUST prevent accidental broadening to all services. Preview scope MUST be rechecked, oversized relation sets MUST fail without partial writes, and history/link handling plus catalog deletion MUST commit atomically. Existing edit/status permissions remain unchanged.
