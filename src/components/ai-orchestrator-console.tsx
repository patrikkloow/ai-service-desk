...
export function AiOrchestratorConsole() {
  ...
  const runtime = useQuery(api.orchestrator.runtimeInfo, isReady ? {} : "skip");
  ...
  const processCustomerMessage = useAction(
  ...
  async function sendToAi(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setStatus(
        ...
        }. ${"metadata" in result && result.metadata ? `${result.metadata.model} · ${result.metadata.mode} · ${result.metadata.latencyMs} ms · ${result.metadata.outcome}` : ""}`,
      );
      if (result.ok) {
        setMessage("");
      } else {
  ...
  return (
    <section className="rounded-xl border border-amber-300 bg-amber-50/50 p-5 lg:col-span-2 dark:border-amber-800 dark:bg-amber-950/20">
      <h2 className="text-lg font-semibold">
        AI orchestrator — serverstyrt testläge
      </h2>
      <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-400">
        Använd syntetiska testuppgifter. Fake/live och modell väljs på servern.
        Live-läge kan utföra riktiga åtgärder i din inloggade organisation.
        Resultatet visar läge, modell, tid och verktyg.
      </p>
      <p className="mt-2 text-sm">
        Läge: {runtime?.mode ?? "laddar"} · Modell: {runtime?.model ?? "—"}
      </p>
      {error !== null ? (
      ...
      <form className="mt-4 grid gap-3" onSubmit={sendToAi}>
        <select
          ...
          onChange={(event) =>
            setConversationId(event.target.value as Id<"conversations">)
          }
          required
      ...
      {conversationId !== "" ? (
        <ul className="mt-5 space-y-2 text-sm">
          {messages?.map((item) => (
            <li
              className="rounded-md bg-white p-3 dark:bg-zinc-900"
              key={item._id}
            >
